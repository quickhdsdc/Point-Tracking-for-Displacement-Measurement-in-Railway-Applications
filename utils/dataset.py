...
IMG_SIZE=256    
def generate_target_reg(joints, image_size=IMG_SIZE):
    joints[0] = joints[0] / image_size - 0.5
    joints[1] = joints[1] / image_size - 0.5
    joints[2] = joints[2] / image_size - 0.5
    joints[3] = joints[3] / image_size - 0.5
    joints[4] = joints[4] / image_size - 0.5
    joints[5] = joints[5] / image_size - 0.5    
    return joints
###############################################################################
#============================ augmentation ===================================#
###############################################################################
...
class ToTensor_reg(object):
    ...
    def __call__(self, sample):
        ...
        image = torch.from_numpy(image)
        if keypoints is not None:
            keypoints = generate_target_reg(keypoints)
            keypoints = torch.from_numpy(keypoints)
            return {'image': image, 'keypoints': keypoints}
        else:
            return {'image': image}        
